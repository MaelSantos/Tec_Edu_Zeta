...
class DesempenhoRepo(BaseRepo):
    def get_by_aluno(self, aluno_id: int) -> List[Desempenho]:
        try:
            return session.query(Desempenho).filter(Desempenho.aluno_id == aluno_id).all()
        except Exception:
    ...
    def get_by_disciplina(self, disciplina_id: int) -> List[Desempenho]:
        try:
            return session.query(Desempenho).filter(Desempenho.disciplina_id == disciplina_id).all()
        except Exception:
